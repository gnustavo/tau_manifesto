...
\end{equation}
Conhecida como a \emph{fórmula de Euler} (em honra a \href{https://pt.wikipedia.org/wiki/Leonhard_Euler}{Leonhard Euler}), esta equação relaciona uma exponencial com argumento imaginário às funções do círculo seno e cosseno e à unidade imaginária~$i$. Embora justificar a fórmula de Euler esteja além do escopo deste manifesto, sua origem está acima de qualquer suspeita e sua importância é inquestionável.
...
\[ U = \int F\,dx = \int_0^x kx\,dx = \textstyle{\frac{1}{2}} kx^2. \]
    ...
    \subsubsection{Energia do movimento} % (fold)
...
\section{Chegando ao fundo de pi e tau} % (fold)
...
Juntando estes resultados, vemos que a Eq.~\eqref{eq:volume_mathworld} pode ser reescrita como
\begin{equation}
\label{eq:volume_2pi}
V_n(r) = \begin{cases}
...
A maior vantagem de $\lambda$ é que ele unifica completamente os casos pares e ímpares nas Eq.~\eqref{eq:surface_area_tau} e Eq.~\eqref{eq:volume_tau}, cada uma das quais tem um fator de $\tau^{\left\lfloor \frac{n}{2} \right\rfloor}$. Substituindo na Eq.~\eqref{eq:tau_lambda} então leva a
\[
\begin{split}
...
Assim, em ambas as Eq.~\eqref{eq:volume_recurrence} e Eq.~\eqref{eq:surface_area_recurrence}, a constante que relaciona as diferentes dimensões não é $\lambda$ propriamente, mas a combinação $2^2\lambda$. Comparando com a Eq.~\eqref{eq:tau_lambda}, vemos que este não é outro que não o~$\tau$! De fato, uma \href{https://en.wikipedia.org/wiki/Volume_of_an_n-ball#The_two-dimension_recursion_formula}{derivação alternativa} da recorrência do volume por cálculo direto (que usa $R$ onde escrevemos $r$) conclui com a integral
\begin{equation}
...
% translate '"SIZE" OF THE INTERIOR' to '"TAMANHO" DO INTERIOR'
% translate "Number of Dimensions" to "Número de Dimensões"
% translate '"SIZE" OF THE BOUNDARY' to '"TAMANHO" DA FRONTEIRA'
\end{center}
...
Equipado com as ferramentas desenvolvidas na Seção~\ref{sec:volume_of_a_hypersphere}, estamos prontos agora para chegar ao fundo da questão sobre $\pi$ e $\tau$. Para concluir a escavação, usaremos as Eq.~\eqref{eq:surface_area_lambda} e Eq.~\eqref{eq:volume_lambda} para definir duas famílias de constantes e, em seguida, usar a definição de $\pi$ (Eq.~\eqref{eq:pi}) para definir uma terceira, revelando assim exatamente o que há de errado com~$\pi$.
...
Vamos fazer algumas observações sobre essas duas famílias de constantes. A família $\tau_n$ tem um significado geométrico importante: fazendo $r=1$ na Eq.~\eqref{eq:surface_area_constants}, vemos que cada $\tau_n$ é a área de superfície de uma $n$-esfera unitária, que também é a medida do ângulo de uma $n$-esfera completa. Em particular, escrevendo $s_n(r)$ como o ``comprimento de arco'' $n$-dimensional correspondente a uma fração~$f$ da área total da superfície~$S_n(r)$, temos
\[
...
Michael tem vergonha de admitir que sabe $\pi$ até 50 casas decimais----\href{\#fig-futurama_video}{aproximadamente 48 a mais que Matt Groening}. Para compensar isso, ele memorizou \href{https://www.wolframalpha.com/input/?i=N[2+Pi,+53]}{52 casas decimais} de~$\tau$.
    ...
    \emph{O Manifesto Tau}. Direitos reservados por Michael Hartl \copyright\ 2010--2018. Versões em e-book de \emph{O Manifesto Tau} estão disponíveis para compra no \href{https://sales.tauday.com/}{site de vendas de O Manifesto Tau}. Sinta-se à vontade para imprimir e distribuir cópias de \emph{O Manifesto Tau} em salas de aula ou usos semelhantes.
